...
typedef struct indices {
	int indLuz, indTemp, indCom, indhum;
} indices;
...
pthread_mutex_t mutex_shidhumdatos = PTHREAD_MUTEX_INITIALIZER; // Mutex buffer datos Humedad
...
void *consolidacion(void *arg) {
	...
	printf("Conectado a MQTT Broker \n");
	if (ret) {
	...
	datosHumedad bufferHumActual[600];
	indices indicesActual;
	int indiceLuz, indiceTemp, indiceCom, indiceHum;
	...
	for (;;) {
		...
		indicesActual.indhum = indicestodos->indhum;

		pthread_mutex_lock(&mutex_shidhumdatos);
		for (int i = 0; i < indiceHum; i++) {
			bufferHumActual[i] = buffer_Hum[i];
		}
		pthread_mutex_unlock(&mutex_shidhumdatos);
...
void *humedad(void *parametros) {
	...
	for (;;) {
		...
		datoActual.datoHumedad = humedad;
		datoActual.tiempo = tiempo;
		pthread_mutex_lock(&mutex_shidhumdatos); //Mutex Buffer Datos Temp Lock
		pthread_mutex_lock(&mutex_shidindices);
		buffer_Hum[indicehum->indhum] = datoActual;
		indicehum->indhum += 1;
		pthread_mutex_unlock(&mutex_shidindices);
		pthread_mutex_unlock(&mutex_shidhumdatos); //Mutex Buffer Datos Temp Unlock
		sleep(frecuenciaHume);
...
// Creacion Instancia sensor Luz
	...
	if (a_pin_luz == NULL) {
		std::cerr << "LUZ Can't create mraa::Aio object, exiting" << std::endl;
		return MRAA_ERROR_UNSPECIFIED;
	}
// Creacion sensor temperatura
	mraa::Aio* a_pin_tmp = new mraa::Aio(1);
	if (a_pin_tmp == NULL) {
		std::cerr << "TEMP Can't create mraa::Aio object, exiting" << std::endl;
		return MRAA_ERROR_UNSPECIFIED;
	}

	//Creacion sensor humedad
	mraa::Aio* a_pin_hum = new mraa::Aio(2);
	if (a_pin_hum == NULL) {
		std::cerr << "HUME Can't create mraa::Aio object, exiting" << std::endl;
		return MRAA_ERROR_UNSPECIFIED;
	}
...
//Instancias de Estructuras
	...
	hum1.pinHum = a_pin_hum;

//Creacion Hilos
	pthread_t HiloLuz, HiloTmp, HiloUsr, HiloConso, HiloHum;
	pthread_create(&HiloHum, NULL, humedad, (void *) &hum1);
